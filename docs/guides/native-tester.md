...
As of v1.5.0 native compilation can be performed and a new set of libraries to facilitate native testing and native "scratch pad" compilation.  `gstio-cc\cpp` and `gstio-ld` now support building "smart contracts" and unit tests natively for quick tests to help facilitate faster development \(note the default implementations of gstio `intrinsics` are currently asserts that state they are unavailable, these are user definable.\)
...
CONTRACT hello : public gstio::contract {
  public:
...
GSTIO_TEST_BEGIN(hello_test)
   ...
   intrinsics::set_intrinsic<intrinsics::read_action_data>(
         [](void* m, uint32_t len) {
            check(len <= sizeof(gstio::name), "failed from read_action_data");
            *((gstio::name*)m) = "hello"_n;
            return len; 
         });

   intrinsics::set_intrinsic<intrinsics::action_data_size>(
         []() {
            return (uint32_t)sizeof(gstio::name);
         });
...
- GSTIO_TEST_BEGIN(X) : This macro defines the beginning of a unit test and assigns `X` as the symbolic name of that test.
- GSTIO_TEST_END : This macro defines the end of a unit test.
- GSTIO_TEST(X) : This is used to run a particular named unit test `X` in the main function.
