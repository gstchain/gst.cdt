...
struct project {
   ...
   const std::string hpp = "#include <gstio/gstio.hpp>\n"
                           "using namespace gstio;\n\n"
   ...
   const std::string cmake = "project(@)\n\n"
                             "set(GSTIO_WASM_OLD_BEHAVIOR \"Off\")\n"
   ...
   const std::string readme_cmake = "--- @ Project ---\n\n"
                                    ...
                                    "   - You can then do a 'set contract' action with 'clgst' and point in to the './build/@' directory\n\n"
                                    " - Additions to CMake should be done to the CMakeLists.txt in the './src' directory and not in the top level CMakeLists.txt";

   const std::string readme_bare = " --- @ Project ---\n\n"
                                   " - How to Build -\n"
                                   "   - run the command 'gstio-cpp -abigen -o @.wasm @.cpp'\n";
...
int main(int argc, const char **argv) {
   ...
   cl::SetVersionPrinter([](llvm::raw_ostream& os) {
        os << "gstio-init version " << "@VERSION_FULL@" << "\n";
  });
   cl::OptionCategory cat("gstio-init", "generates an gstio smart contract project");
   ...
   cl::ParseCommandLineOptions(argc, argv, std::string("gstio-proj"));
   try {
