...
#include <gstio/gen.hpp>

#include <gstio/utils.hpp>
#include <gstio/whereami/whereami.hpp>
#include <gstio/abi.hpp>
...
using namespace gstio::cdt;

namespace gstio { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "gstio.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_gstiolib = false;

   class gstio_ppcallbacks : public PPCallbacks {
      public:
         gstio_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("gstiolib") != StringRef::npos )
               has_gstiolib = true;
         }
   ...
   class gstio_codegen_visitor : public RecursiveASTVisitor<gstio_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit gstio_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class gstio::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_gstio_contract(decl, cg.contract_name)) {
               if (has_gstiolib) {
                  ss << "\n\n#include <gstiolib/datastream.hpp>\n";
                  ss << "#include <gstiolib/name.hpp>\n";
               } else {
                  ss << "\n\n#include <gstio/datastream.hpp>\n";
                  ss << "#include <gstio/name.hpp>\n";
               }
               ss << "extern \"C\" {\n";
               ...
               ss << "}\n";
               ss << "gstio::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("gstio_wasm_action", "__gstio_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("gstio_wasm_notify", "__gstio_notify_", func, decl);
         }
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isGstioAction()) {
               name = generation_utils::get_action_name(decl);
               validate_name(name, [&]() {emitError(*ci, decl->getLocation(), "action not a valid gstio name");});
      ...
      class gstio_codegen_consumer : public ASTConsumer {
      private:
         gstio_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit gstio_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new gstio_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void gstio_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, gstio_wasm_entry, gstio_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_gstio_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "gstio_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class gstio_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<gstio_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<gstio_codegen_consumer>(&CI, file);
         }
   };
