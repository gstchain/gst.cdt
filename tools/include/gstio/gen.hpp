...
struct generation_utils {
      ...
      auto check = [&](const clang::Type* pt) {
        if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
         if (auto rt = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
            return rt->getDecl()->isGstioIgnore();
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isGstioIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_gstio_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasGstioRicardian();
   }
   static inline bool has_gstio_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasGstioRicardian();
   }

   static inline std::string get_gstio_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getGstioRicardianAttr()->getName();
   }
   static inline std::string get_gstio_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getGstioRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getGstioActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang::CXXMethodDecl* decl ) {
      ...
      auto tmp = decl->getGstioNotifyAttr()->getName();
      return tmp;
   }
   ...
   static inline bool is_gstio_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isGstioContract())
         name = decl->getGstioContractAttr()->getName();
      else if (decl->getParent()->isGstioContract())
         name = decl->getParent()->getGstioContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_gstio_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isGstioContract()) {
         auto nm = decl->getGstioContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isGstioContract()) {
         auto nm = pd->getGstioContractAttr()->getName().str();
