...
Error WasmObjectFile::parseGstioABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   gstio_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("gstio abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".gstio_abi") {
     if (Error Err = parseGstioABISection(Ctx))
        return Err;
  } else if (Sec.Name == ".gstio_actions") {
     if (Error Err = parseActionsSection(Ctx))
        return Err;
  } else if (Sec.Name == ".gstio_notify") {
