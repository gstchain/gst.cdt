...
namespace {
  // GstioApply - Mutate the apply function as needed 
  struct GstioApplyPass : public FunctionPass {
    static char ID; 
    GstioApplyPass() : FunctionPass(ID) {}
    bool runOnFunction(Function &F) override {
       if (F.hasFnAttribute("gstio_wasm_entry") || F.getName().equals("apply")) {
...
char GstioApplyPass::ID = 0;
static RegisterPass<GstioApplyPass> X("apply_fixup", "Gstio Apply Fixups");

static void registerGstioApplyPass(const PassManagerBuilder&, legacy::PassManagerBase& PM) { PM.add(new GstioApplyPass()); }
static RegisterStandardPasses RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible, registerGstioApplyPass);
