...
void WebAssemblyAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  bool has_abi    = false;
  bool has_gstio_action = false;
  bool has_gstio_notify = false;
  for (const auto &F : M) {
     if (F.hasFnAttribute("gstio_wasm_import"))
        has_import = true;
     if (F.hasFnAttribute("gstio_wasm_abi"))
        has_abi = true;
     if (F.hasFnAttribute("gstio_wasm_action"))
        has_gstio_action = true;
     if (F.hasFnAttribute("gstio_wasm_notify"))
        has_gstio_notify = true;
  }
  ...
  if (has_abi) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".gstio_abi";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("gstio_wasm_abi")) {
           StringRef abi = F.getFnAttribute("gstio_wasm_abi").getValueAsString();
  ...
  if (has_gstio_action) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".gstio_actions";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("gstio_wasm_action")) {
           StringRef action_name = F.getFnAttribute("gstio_wasm_action").getValueAsString();
  ...
  if (has_gstio_notify) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".gstio_notify";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("gstio_wasm_notify")) {
           StringRef name = F.getFnAttribute("gstio_wasm_notify").getValueAsString();
