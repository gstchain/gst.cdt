...
llvm::Constant *CodeGenModule::GetOrCreateLLVMFunction(
  ...
  if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
     if (FD->hasAttr<GstioWasmImportAttr>())
        isWasmImport = true;
     if (FD->hasAttr<GstioWasmEntryAttr>())
        isWasmEntry = true;
     if (FD->hasAttr<GstioWasmABIAttr>())
        isWasmABI = true;
     if (FD->hasAttr<GstioWasmActionAttr>())
        isWasmAction = true;
     if (FD->hasAttr<GstioWasmNotifyAttr>())
        isWasmNotify = true;
  ...
  if (isWasmImport)
   F->addFnAttr("gstio_wasm_import", "true");

  if (isWasmEntry)
   F->addFnAttr("gstio_wasm_entry", "true");
     
  if (isWasmABI)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("gstio_wasm_abi", FD->getWasmABI().c_str());
     }
  if (isWasmAction)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("gstio_wasm_action", FD->getGstioWasmAction().c_str());
     }
  if (isWasmNotify)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("gstio_wasm_notify", FD->getGstioWasmNotify().c_str());
     }
