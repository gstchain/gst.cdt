...
inline bool expect_assert(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   if (ret == 0) {
      ...
      if (!check)
         gstio::check(false, std::string("error : expect_assert, no assert {"+li+"}").c_str());
   ...
   silence_output(false);
   if (!check)
      gstio::check(passed, std::string("error : expect_assert, wrong assert {"+li+"}").c_str());
   if (!passed)
...
inline bool expect_print(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   silence_output(false);
   if (!check)
      gstio::check(passed, std::string("error : wrong print message {"+li+"}").c_str());
   if (!passed)
...
#define REQUIRE_EQUAL(X, Y) \
   gstio::check(X == Y, std::string(std::string("REQUIRE_EQUAL failed (")+#X+" != "+#Y+") {"+__FILE__+":"+std::to_string(__LINE__)+"}").c_str());
