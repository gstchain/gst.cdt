...
#warning "<gstiolib/name.hpp> is deprecated use <gstio/name.hpp>"
namespace gstio {
   ...
   public:
      ...
      {
         if( str.size() > 13 ) {
            gstio::check( false, "string is too long to be a valid name" );
         }
         if( str.empty() ) {
            return;
         }
         ...
         if( str.size() == 13 ) {
            ...
            if( v > 0x0Full ) {
               gstio::check(false, "thirteenth character in name cannot be a letter that comes after j");
            }
            value |= v;
         }
      }
      ...
      static constexpr uint8_t char_to_value( char c ) {
         if( c == '.')
            ...
            gstio::check( false, "character is not in allowed character set for names" );
...
inline constexpr gstio::name operator""_n() {
   constexpr auto x = gstio::name{std::string_view{gstio::detail::to_const_char_arr<Str...>::value, sizeof...(Str)}};
   return x;
}
