...
#include "../../contracts/gstio/action.hpp"
#include "../../core/gstio/name.hpp"
#include "../../core/gstio/serialize.hpp"
#include "../../core/gstio/fixed_bytes.hpp"
...
namespace _multi_index_detail {
   ...
   WRAP_SECONDARY_ARRAY_TYPE(idx256, gstio::fixed_bytes<32>)
   template<>
   struct secondary_key_traits<gstio::fixed_bytes<32>> {
      static constexpr gstio::fixed_bytes<32> true_lowest() { return gstio::fixed_bytes<32>(); }
   };

}

/**
 *  The indexed_by struct is used to instantiate the indices for the Multi-Index table. In GSTIO, up to 16 secondary indices can be specified.
 *
 *  @ingroup multiindex
 *  @tparam IndexName - is the name of the index. The name must be provided as an GSTIO base32 encoded 64-bit integer and must conform to the GSTIO naming requirements of a maximum of 13 characters, the first twelve from the lowercase characters a-z, digits 1-5, and ".", and if there is a 13th character, it is restricted to lowercase characters a-p and ".".
 *  @tparam Extractor - is a function call operator that takes a const reference to the table object type and returns either a secondary key type or a reference to a secondary key type. It is recommended to use the `gstio::const_mem_fun` template, which is a type alias to the `boost::multi_index::const_mem_fun`. See the documentation for the Boost `const_mem_fun` key extractor for more details.
 *
 *  Example:
       *
*
 ...
 *  class mycontract: gstio::contract {
 *    struct record {
 *       uint64_t    primary;
 *       uint128_t   secondary;
 ...
 *      }
 *  }
 *  GSTIO_DISPATCH( mycontract, (myaction) )
 *  @endcode
 */
 ...
 *  @ingroup contracts
 *
 *  @brief Defines GSTIO Multi Index Table
 *  @details GSTIO Multi-Index API provides a C++ interface to the GSTIO database. It is patterned after Boost Multi Index Container.
 *  GSTIO Multi-Index table requires exactly a uint64_t primary key. For the table to be able to retrieve the primary key,
 *  the object stored inside the table is required to have a const member function called primary_key() that returns uint64_t.
 *  GSTIO Multi-Index table also supports up to 16 secondary indices. The type of the secondary indices could be any of:
 *  - uint64_t
 *  - uint128_t
 *  - double
 *  - long double
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     gstio::check( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        gstio::check( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               gstio::check( lb != cend(), error_msg );
               gstio::check( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               gstio::check( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               gstio::check( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            const_iterator erase( const_iterator itr ) {
               gstio::check( itr != cend(), "cannot pass end iterator to erase" );
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         gstio::check( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `gstio::multi_index` template has template parameters `<name::raw TableName, typename T, typename... Indices>`, where:
       *  - `TableName` is the name of the table, maximum 12 characters long, characters in the name from the set of lowercase letters, digits 1 to 5, and the "." (period) character and is converted to a gstio::raw - which wraps uint64_t;
       ...
       *  - It is recommended to use the gstio::const_mem_fun template, which is a type alias to the boost::multi_index::const_mem_fun.  See the documentation for the Boost const_mem_fun key extractor for more details.
       *
       *  Example:
       *
       *  @code
       ...
       *      typedef gstio::multi_index< "address"_n, address > address_index;
       *      void myaction() {
       ...
       *        gstio::check(addresses.get_code() == "dan"_n, "Codes don't match.");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses("dan"_n, "dan"_n); // code, scope
       *        gstio::check(addresses.get_code() == "dan"_n, "Scopes don't match");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               auto ei = internal_use_do_not_use::db_end_i64(_multidx->get_code(), _multidx->get_scope(), static_cast<uint64_t>(TableName));
               ...
               gstio::check( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *        gstio::check(itr == addresses.cbegin(), "Only address is not at front.");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        gstio::check(itr == addresses.begin(), "Only address is not at front.");
       *      }
       *  }
       *  GSTIO_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator begin()const  { return cbegin(); }
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        gstio::check(itr != addresses.cend(), "Address for account doesn't exist");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        gstio::check(itr != addresses.end(), "Address for account doesn't exist");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *          address.street = "1 GST Way";
       *          address.city = "Hong Kong";
       *          address.state = "HK";
       *        });
       *        auto itr = addresses.crbegin();
       *        gstio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Last Record ");
       *        itr++;
       *        gstio::check(itr->account_name == name("brendan"), "Lock arf, Incorrect Second Last Record");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        gstio::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Record ");
       *        itr--;
       *        gstio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Record");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        gstio::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Lower Bound Record ");
       *        itr++;
       *        gstio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Lower Bound Record");
       *        itr++;
       *        gstio::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator lower_bound( uint64_t primary )const {
         auto itr = internal_use_do_not_use::db_lowerbound_i64( _code, _scope, static_cast<uint64_t>(TableName), primary );
         if( itr < 0 ) return end();
       ...
       *        gstio::check(itr->account_name == name("dan"), "Lock arf, Incorrect First Upper Bound Record ");
       *        itr++;
       *        gstio::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *      }
       *  }
      ...
      const_iterator upper_bound( uint64_t primary )const {
         auto itr = internal_use_do_not_use::db_upperbound_i64( _code, _scope, static_cast<uint64_t>(TableName), primary );
         if( itr < 0 ) return end();
         const auto& obj = load_object_by_primary_iterator( itr );
         return {this, &obj};
      }
      ...
      uint64_t available_primary_key()const {
         ...
         gstio::check( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *      typedef gstio::multi_index< name("address"), address, indexed_by< name("zip"), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        // create reference to address_index  - see emplace example below
       *        // add dan account to table           - see emplace example below
       ...
       *        gstio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Record ");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        gstio::check(iterator_to(user) == itr, "Invalid iterator");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         gstio::check( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( uint64_t payer, Lambda&& constructor ) {
         ...
         auto itm = std::make_unique<item>( this, [&]( auto& i ){
            ...
            i.__primary_itr = internal_use_do_not_use::db_store_i64( _scope, static_cast<uint64_t>(TableName), payer, pk, buffer, size );
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        gstio::check(itr != addresses.end(), "Address for account not found");
       ...
       *          address.state = "CA";
       *        });
       *        gstio::check(itr->city == "San Luis Obispo", "Lock arf, Address not modified");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, uint64_t payer, Lambda&& updater ) {
         ...
         gstio::check( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
         ...
         internal_use_do_not_use::db_update_i64( objitem.__primary_itr, payer, buffer, size );
       ...
       *
       *        auto user = addresses.get("dan"_n);
       *        gstio::check(user.first_name == "Daniel", "Couldn't get him.");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        gstio::check(itr != addresses.end(), "Couldn't get him.");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator find( uint64_t primary )const {
         ...
         auto itr = internal_use_do_not_use::db_find_i64( _code, _scope, static_cast<uint64_t>(TableName), primary );
         if( itr < 0 ) return end();
       ...
       *        gstio::check(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        gstio::check(itr != addresses.end(), "Everting lock arf, Address not erased properly");
       *      }
       *  }
       *  GSTIO_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator erase( const_iterator itr ) {
         gstio::check( itr != end(), "cannot pass end iterator to erase" );
       ...
       *      void myaction() {
       *        auto itr = addresses.find("dan"_n);
       *        gstio::check(itr != addresses.end(), "Record is not found");
       *        addresses.erase(*itr);
       *        itr = addresses.find("dan"_n);
       *        gstio::check(itr == addresses.end(), "Record is not deleted");
       *      }
       *  }
       *  GSTIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         gstio::check( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
