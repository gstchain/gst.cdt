...
#include "core/gstio/powers.hpp"
#include "contracts/gstio/system.hpp"
#include "contracts/gstio/privileged.hpp"
...
namespace gstio {
   ...
   // privileged.hpp
   void set_blockchain_parameters(const gstio::blockchain_parameters& params) {
      char buf[sizeof(gstio::blockchain_parameters)];
      gstio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(gstio::blockchain_parameters& params) {
      char buf[sizeof(gstio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      gstio::check( size <= sizeof(buf), "buffer is too small" );
      gstio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }

   std::optional<uint64_t> set_proposed_producers( const std::vector<producer_key>& prods ) {
      auto packed_prods = gstio::pack( prods );
