...
#warning "<gstiolib/system.hpp> is deprecated use <gstio/system.hpp>"
namespace gstio {
   ...
   inline void check(bool pred, const char* msg, size_t n) {
      if (!pred) {
         gstio_assert_message(false, msg, n);
      }
   }
   ...
   inline void check(bool pred, const std::string& msg, size_t n) {
      if (!pred) {
         gstio_assert_message(false, msg.c_str(), n);
      }
   }
