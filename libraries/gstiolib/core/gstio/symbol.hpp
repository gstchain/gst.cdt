...
namespace gstio {
   ...
   */
   ...
   public:
      ...
      {
         if( str.size() > 7 ) {
            gstio::check( false, "string is too long to be a valid symbol_code" );
         }
         for( auto itr = str.rbegin(); itr != str.rend(); ++itr ) {
            if( *itr < 'A' || *itr > 'Z') {
               gstio::check( false, "only uppercase letters allowed in symbol_code string" );
            }
            value <<= 8;
            value |= *itr;
         }
   ...
   inline DataStream& operator<<(DataStream& ds, const gstio::symbol_code sym_code) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, gstio::symbol_code& sym_code) {
     uint64_t raw = 0;
   ...
   public:
      ...
      void print( bool show_precision = true )const {
         if( show_precision ){
            ::gstio::print( static_cast<uint64_t>(precision()), "," );
         }
         char buffer[7];
    ...
    */
   template<typename DataStream>
   inline DataStream& operator<<(DataStream& ds, const gstio::symbol sym) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, gstio::symbol& sym) {
     uint64_t raw = 0;
     ds.read((char*)&raw, sizeof(raw));
     sym = symbol(raw);
     return ds;
   ...
   private:
      ...
      GSTLIB_SERIALIZE( extended_symbol, (symbol)(contract) )
   };
}
