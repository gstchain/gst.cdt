...
#include <gstio/binary_extension.hpp>
#include <gstio/crypto.hpp>
#include <gstio/datastream.hpp>
#include <gstio/ignore.hpp>
#include <gstio/symbol.hpp>
...
using gstio::binary_extension;
using gstio::datastream;
using gstio::fixed_bytes;
using gstio::ignore;
using gstio::ignore_wrapper;
using gstio::pack;
using gstio::pack_size;
using gstio::public_key;
using gstio::signature;
using gstio::symbol;
using gstio::symbol_code;
using gstio::unpack;
...
struct be_test {
   ...
   be_test(int i) : val{i} {}
   int val;
   GSTLIB_SERIALIZE( be_test, (val) )
};

// Definitions in `gstio.cdt/libraries/gstio/datastream.hpp`
GSTIO_TEST_BEGIN(datastream_test)
...
// Definitions in `gstio.cdt/libraries/gstio/datastream.hpp`
GSTIO_TEST_BEGIN(datastream_specialization_test)
   static constexpr uint16_t buffer_size{256};
   char datastream_buffer[buffer_size]{}; // Buffer for the datastream to point to
   char buffer[buffer_size]; // Buffer to compare `datastream_buffer` with
...
// Definitions in `gstio.cdt/libraries/gstio/datastream.hpp`
GSTIO_TEST_BEGIN(datastream_stream_test)
   static constexpr uint16_t buffer_size{256};
   char datastream_buffer[buffer_size]; // Buffer for the datastream to point to
...
// Definitions in `gstio.cdt/libraries/gstio/datastream.hpp`
GSTIO_TEST_BEGIN(misc_datastream_test)
   // ---------------------------
...
int main(int argc, char* argv[]) {
   ...
   GSTIO_TEST(datastream_test);
   GSTIO_TEST(datastream_specialization_test);
   GSTIO_TEST(datastream_stream_test);
   GSTIO_TEST(misc_datastream_test);
   return has_failed();
}
