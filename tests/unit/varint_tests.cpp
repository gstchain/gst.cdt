...
#include <gstio/varint.hpp>

using std::numeric_limits;

using gstio::datastream;
using gstio::unsigned_int;
using gstio::signed_int;
...
// Defined in `gstio.cdt/libraries/gstio/varint.hpp`
GSTIO_TEST_BEGIN(unsigned_int_type_test)
...
// Defined in `gstio.cdt/libraries/gstio/varint.hpp`
GSTIO_TEST_BEGIN(signed_int_type_test)
   //// signed_int(uint32_t)
...
int main(int argc, char* argv[]) {
   ...
   GSTIO_TEST(unsigned_int_type_test)
   GSTIO_TEST(signed_int_type_test);
   return has_failed();
}
