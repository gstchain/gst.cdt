...
// Definitions in `gstio.cdt/libraries/gstio/name.hpp`
GSTIO_TEST_BEGIN(name_type_test)
   //// constexpr name()
   ...
   CHECK_EQUAL( name{"gst"}.length(), 3 )
   CHECK_EQUAL( name{"gsti"}.length(), 4 )
   CHECK_EQUAL( name{"gstio"}.length(), 5 )
   CHECK_EQUAL( name{"gstioa"}.length(), 6 )
   CHECK_EQUAL( name{"gstioac"}.length(), 7 )
   CHECK_EQUAL( name{"gstioacc"}.length(), 8 )
   CHECK_EQUAL( name{"gstioacco"}.length(), 9 )
   CHECK_EQUAL( name{"gstioaccou"}.length(), 10 )
   CHECK_EQUAL( name{"gstioaccoun"}.length(), 11 )
   CHECK_EQUAL( name{"gstioaccount"}.length(), 12 )
   CHECK_EQUAL( name{"gstioaccountj"}.length(), 13 )

   CHECK_ASSERT( "string is too long to be a valid name", ([]() {name{"12345abcdefghj"}.length();}) )

   // ----------------------------
   // constexpr name suffix()const
   CHECK_EQUAL( name{".gstioaccounj"}.suffix(), name{"gstioaccounj"} )
   CHECK_EQUAL( name{"e.osioaccounj"}.suffix(), name{"osioaccounj"} )
   CHECK_EQUAL( name{"eo.sioaccounj"}.suffix(), name{"sioaccounj"} )
   CHECK_EQUAL( name{"gst.ioaccounj"}.suffix(), name{"ioaccounj"} )
   CHECK_EQUAL( name{"gsti.oaccounj"}.suffix(), name{"oaccounj"} )
   CHECK_EQUAL( name{"gstio.accounj"}.suffix(), name{"accounj"} )
   CHECK_EQUAL( name{"gstioa.ccounj"}.suffix(), name{"ccounj"} )
   CHECK_EQUAL( name{"gstioac.counj"}.suffix(), name{"counj"} )
   CHECK_EQUAL( name{"gstioacc.ounj"}.suffix(), name{"ounj"} )
   CHECK_EQUAL( name{"gstioacco.unj"}.suffix(), name{"unj"} )
   CHECK_EQUAL( name{"gstioaccou.nj"}.suffix(), name{"nj"} )
   CHECK_EQUAL( name{"gstioaccoun.j"}.suffix(), name{"j"} )

   CHECK_EQUAL( name{"e.o.s.i.o.a.c"}.suffix(), name{"c"} )
   CHECK_EQUAL( name{"gst.ioa.cco"}.suffix(), name{"cco"} )
